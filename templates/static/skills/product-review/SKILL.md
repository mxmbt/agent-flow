...
- `{{artifacts.productFile}}` — продуктовое видение
- `{{artifacts.architectureFile}}` — технический план / constraints
- `{{artifacts.roadmapFile}}` — роадмап
- `{{artifacts.uiUxSpecificationFile}}` — screen hierarchy и journey context (если есть UI scope)
- Результаты Codebase Research (если есть; при standalone — изучи кодовую базу самостоятельно через Grep/Glob/Read)
- Runtime memory/context загружается согласно активной среде агента

## Pre-Review Audit (перед Step 0)

Прежде чем review-ить — пойми текущее состояние:

1. Изучи кодовую базу: главные конфиги, структуру проекта, data/schema definitions если они есть
...
3. **Что произойдёт если НЕ делать?** Реальная боль или гипотетическая?
   (сверь с `{{artifacts.productFile}}` target audience и `{{artifacts.uiUxSpecificationFile}}`, если задача user-facing)
...
* Internal (docs/, {{target.toolRoot}}/, agent/tooling config, infra/) → автоматически HOLD
...
## Section 2: Persona & Journey Mapping (если user-facing)

- Какая персона — primary user? (сверь с `{{artifacts.productFile}}`)
- На каком этапе продукта находится этот экран/функция? (dashboard / workflow / collection / profile / ops)
...
Если задача имеет значительный UI scope — рекомендуй прочитать `{{artifacts.designSystemFile}}`
и `{{artifacts.uxWritingGuideFile}}` в IMPL phase.
...
Верни структурированный результат в формате:
```markdown
### Premise Challenge
[ответы на 3 вопроса]
...
### Persona & Journey Context (если user-facing)
[маппинг]
