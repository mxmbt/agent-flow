...
def rebuild_colors():
    ...
    with open(src, newline="", encoding="utf-8") as f:
        reader = csv.DictReader(line for line in f if not line.startswith("# -flow managed "))
    ...
    with open(os.path.join(BASE, "products.csv"), newline="", encoding="utf-8") as f:
        products = list(csv.DictReader(line for line in f if not line.startswith("# -flow managed ")))
