...
const GENERATE_TOKENS_SCRIPT = '.claude/skills/design-system-uupm/scripts/generate-tokens.cjs';
...
function updateDesignTokens(tokens, colors) {
  // Update brand name
  const brandName = `Project Brand - ${colors.primary.name.split('-').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')}`;
  ...
  if (tokens.semantic?.color) {
    ...
    // Finance-style semantics: bullish = success, bearish = error, primary = info, secondary = warning
    sem.success = { "$value": `{primitive.color.${a}.500}`, "$type": "color" };
    sem['success-light'] = { "$value": `{primitive.color.${a}.300}`, "$type": "color" };
    sem.error = { "$value": `{primitive.color.bearish-red.500}`, "$type": "color" };
    sem['error-light'] = { "$value": `{primitive.color.bearish-red.500}`, "$type": "color" };
    sem.warning = { "$value": `{primitive.color.${s}.500}`, "$type": "color" };
    sem['warning-light'] = { "$value": `{primitive.color.${s}.300}`, "$type": "color" };
    sem.info = { "$value": `{primitive.color.${p}.500}`, "$type": "color" };
    sem['info-light'] = { "$value": `{primitive.color.${p}.300}`, "$type": "color" };
  }
