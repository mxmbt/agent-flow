...
function extractVoice(content) {
  ...
  if (personalityMatch) {
    ...
    if (traits) {
      ...
      }).filter((value) => value && value.toLowerCase() !== "trait");
    }
  }
...
function extractCoreAttributes(content) {
  ...
  return attributes.filter((item) => item.name.toLowerCase() !== "attribute");
}
    ...
    if (keywordRows) {
      keywordRows.forEach((row) => {
        ...
        if (match) {
          ...
          imageStyle.keywords.push(
            ...keywords.filter((keyword) => keyword.toLowerCase() !== "keywords")
          );
        }
      });
    }
