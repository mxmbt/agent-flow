...
### Step 1: Create Explicit Work Plan

Create a concrete TODO checklist in the current session. Break down into:
...
Parallelize only truly independent tasks (for example multiple HTML files or exports for different sizes).
...
5. Save to `assets/social-photos/exports/`
...
Save report to `.context/design-reports/social-photos/` or another user-requested output path.
...
- Move/copy exported PNGs to `assets/social-photos/`
- Keep transient HTML working files under `.context/` if they are not meant for commit
- Clean up intermediate files if requested
- Tag outputs with metadata (platform, size, concept name) in the report
